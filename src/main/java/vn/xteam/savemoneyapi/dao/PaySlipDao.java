...
public class PaySlipDao implements IBaseDao<PaySlipEntity> {
    ...
    public List<PaySlipEntity> findAll() {
        ...
        try {
            ...
            String query = String.format("SELECT p.id, p.debit_money, p.created_at, p.updated_at, p.customer_code,p.created_by, c.full_name, c.address ,s.code FROM `%s` as p LEFT JOIN customers as c ON p.customer_id = c.id LEFT JOIN saving_books as s ON p.saving_book_id = s.id order by p.created_at desc;", TABLE_NAME);
            LOGGER.info("query: ");
